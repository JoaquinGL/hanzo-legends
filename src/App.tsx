...
export default function App() {
  ...
  const [isFinalAnimationActive, setIsFinalAnimationActive] =
    useState<boolean>(false);
      ...
      >
        ...
        <header
          ...
          style={{
            opacity: selectedChar
              ? Math.max(0, 1 - transitionProgress * 3.0)
              : 1,
        ...
        */}
        <div
          ...
          className='fixed bottom-0 left-0 right-0 flex justify-center items-end origin-bottom bg-gradient-to-t from-slate-100/95 via-slate-50/45 to-transparent pt-12 pb-0 px-6 pointer-events-none'
          style={{
            opacity: selectedChar
        ...
        >
          <div className='flex items-end justify-center gap-2 sm:gap-4 pointer-events-auto pb-0'>
            {characters.map((char, index) => (
              <SoundBar
                ...
                isTotallyHidden={
                  isFinalAnimationActive ||
                  (isTransitioning && selectedChar?.id === char.id)
                }
                onClick={() => {
                  const el = document.getElementById(`bar-main-${char.id}`);
                  const rect = el?.getBoundingClientRect();
